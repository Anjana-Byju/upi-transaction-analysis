...
cat("Correct (False): ", sum(df$KC_Score), "/", nrow(df), "=",
    round(mean(df$KC_Score) * 100, 1), "%\n")
cat("Incorrect (True):", sum(1 - df$KC_Score), "/", nrow(df), "=",
...
age_df$Percent <- round(age_df$Count / nrow(df) * 100, 1)
...
gender_df$Percent <- round(gender_df$Count / nrow(df) * 100, 1)
...
conf_df$Percent <- round(conf_df$Count / nrow(df) * 100, 1)
...
aware_df$Percent <- round(aware_df$Count / nrow(df) * 100, 1)
...
security_items <- data.frame(
  ...
  No_Count  = c(nrow(df) - sum(df$AppLock_Code), nrow(df) - sum(df$PIN_Safe), nrow(df) - sum(df$WiFi_Safe))
)
...
security_long$Percent  <- round(security_long$Count / nrow(df) * 100, 1)
